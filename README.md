...
后续提交不可修改cache规格，只能根据report反馈修复bug。SoC和后端团队将定期检查新提交的代码，进行综合和仿真测试，并将结果以日志报告的形式上传至[ysyx_submit仓库](https://github.com/OSCPU/ysyx_submit/tree/main/report)。代码提交后请每天检查该仓库中对应学号的`.txt`文件，以了解当前的代码是否能够正确通过综合和仿真测试。对于未能正确通过综合和仿真测试的代码，请根据报告反馈修改代码并重新提交，`一生一芯`项目组原则上不修改提交的代码。对于已通过综合和仿真测试的代码，禁止再次提交。
