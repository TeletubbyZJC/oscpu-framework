...
完成代码上传操作后，组内助教会将仓库URL进行汇总。
团队在深圳鹏城实验室支撑单位部署了gitlab服务器, 并搭建了一套CI/CD流程用于自动化进行综合与测试, 其具体流程如下:
1. 对汇总得到的仓库URL列表进行拉取, 并对比仓库的时间戳和上一次综合测试报告的时间戳，若时间戳有更新，则触发综合与测试流程
1. 综合和测试流程包括以下几步(每步都会分别生成相应的报告):
  * 单处理器DC综合
  * 单处理器VCS测试
  * 集成流片SoC环境DC综合
  * 集成流片SoC环境VCS测试
1. 对报告根据时间戳进行重命名并打包，其中DC报告的文件名为`dc_report`, VCS报告的文件名为`vcs_report`, 打包的report文件夹为个人仓库名加commit 时间，如`ysyx_210888-Wed Sep 29 11:14:07 2021 +0800`
1. report会在本批次拉取的所有版本代码都测试完成之后push到主仓库指定文件夹下。主仓库的report存贮文件夹下的第一级文件夹名字的名字为 DC&VCS-[pull date]形式，如果commit时间在文件名中的[pull date]之前不久，则可去检查是否有自己的报告；次一级文件夹即上述第三步生成的report打包文件夹，命名形式为ysyx_xxxxxx-[commit date]。
1. 前端人员需要在报告回传之后根据报告中报出的Warning和Error进行代码修改，多次不能修改的Warning和Error需要以指定形式记录问题的详细信息报告给组长，组长整理完只有交给后端和SOC支撑人员，后续统一处理。
1. 本次拉取的所有未测试版本代码测试完并回传完report之后，就会重新拉取最新版本代码，开始下一个CI/CD流程。
